...
2. 测试夹具里的账号 id、命名空间、代金券码必须是**编造**的。
   不要从真实响应里照抄 —— 命名空间、账号 id 这类值单看不敏感，但组合起来能定位到具体账号。
3. 本仓库**历史上曾误提交过真实令牌**，已用 `git filter-repo` 重写全部历史清除。
   重写代价极高：所有 commit SHA 变更、已关闭 issue / PR 里引用的旧 SHA 全部失效，
   还要强推 + 通知所有协作者重新克隆。**不要重蹈覆辙。**

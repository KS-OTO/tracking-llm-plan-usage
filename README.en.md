...
| Volcano Ark Agent Plan           | `GetAFPUsage` + `GetUsageDetails` + `GetCodingPlanUsage` + `GetInferenceUsage`                                                                                                                                                | The card shows the Agent Plan's 5-hour/daily/weekly/monthly quotas and progress plus reset countdown; plan type, Coding Plan status and window quotas, model call details and inference usage for the last N days (filterable by model) all live in the details dialog     |
...
server/           platform-agnostic API core + Bun entry point
  ...
  volc.ts         Ark control-plane API client (GetAFPUsage / GetCodingPlanUsage / GetUsageDetails / GetInferenceUsage)

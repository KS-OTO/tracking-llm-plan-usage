...
| Volcano Ark Agent Plan           | `GetPersonalPlan` + `GetAFPUsage` + `GetUsageDetails` + `GetCodingPlanUsage` + `GetInferenceUsage`                                                                                                                            | The card shows the Agent Plan's 5-hour/model-daily/weekly/monthly quotas and progress plus reset countdown; plan tier and validity, Coding Plan status and window quotas, model call details and inference usage for the last N days (filterable by model) all live in the details dialog |

...
5. **文字色不许取「填充色」token**：`--td-brand-color` 是填充档（按钮底 / 进度条 / 开关），
   ...
   2.15:1**，而且所有门禁都是绿的。链接一律走 `--td-text-color-brand` / `-link`。
   本项目实测有三处（`layout.css` 的 `.models-link` / `.detail-links a`、`main.css` 的全局
   `a`），**是静态守卫抓出来的、不是人眼** —— 守卫：`theme.test.ts` 的「前景色属性不得直接
   引用填充档 token」（只锁 `color` 一类前景属性，`background` 用填充档是合法的）。

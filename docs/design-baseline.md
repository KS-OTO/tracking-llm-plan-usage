...
取值来自 18Bit 现网站点的**计算后样式**，不是目测。主依据是 `help-dev.18bit.cn/result`
（DNS 检测结果页）—— 它才是与本项目同类的「密集读数面板」，另两个（`go.18bit.cn`、
`go.18bit.cn/help-docs/`）是营销/文档页，只用于交叉印证：

| 角色                | 参考站实测值                             | 落点（取值以 `theme.css` 为准）                                                  |
| ------------------- | ---------------------------------------- | -------------------------------------------------------------------------------- |
| 页面底色            | `#16171b`                                | 暗色 `--td-bg-color-page`                                                        |
| 内容面板（大卡）    | `#000000`（**比页面更深**）              | **没有照抄**，改用 `#1c1d22`，理由见 `theme.css` 文件头「三处刻意的偏离」第 1 条 |
| 大卡内部元素 / 分块 | `#1c1d22`（**比页面浅**）                | 暗色 `--td-bg-color-container`                                                   |
| 抬起的控件 / 分隔   | `#292b32`（全站 56 处）                  | 暗色 `secondarycontainer` / `component`                                          |
| 主色（按钮底）      | `#0d51d9`，hover `#4678f9`               | 品牌阶 anchor：亮 -7 / **暗 -8 换成 `#4e7ef9`**，见规矩 5                        |
| 强调绿              | `#21e786`（8 档 alpha 成套）             | 成功阶 anchor，**亮暗都落在 -5**，见规矩 6                                       |
| 警告琥珀 / 危险粉   | `#ffc55a` / `#ff5fa0`                    | 警告 -5 / 危险 -6（亮色朝黑压深，见规矩 6）                                      |
| 标题 / 次要 / 更淡  | `#f9faff` / `#959cb1` / `#6a6f82`        | 暗色三档文字                                                                     |
| 链接 / 品牌文字     | 亮 `#0d51d9`、暗 `#6991fa`               | `--td-text-color-brand` / `-link`                                                |
| 卡片描边            | 卡片 `1px solid #ffffff`                 | **没有照抄**，降到 24% 白，理由见 `theme.css` 文件头「三处刻意的偏离」第 2 条    |
| 圆角                | **20 / 12 / 10 / 8 / 6 / 2**（阶梯）     | 按消费方落到五档，见 `theme.css`「圆角是阶梯」                                   |
| 阴影                | `0 20px 48px #16171bcc` 等三个（大而软） | 三个 shadow token（与上一版「收紧成贴边」相反）                                  |
| 字体                | `system-ui, -apple-system, …`            | `--td-font-family`（参考站已放弃在线字体）                                       |

`#21e786` 在 `/result` 里是**一整套 8 档 alpha 阶梯**（容器底 / 元素底 / 提示条 / 激活底与
表头 / hover / hover 边框 / 激活边框 / 徽标底），不是零星点缀 —— 所以它是**语义色**，
不是插画色。这条判断直接推翻了上一版的规矩 4。

六条规矩：

1. **新增视觉值一律先进 `theme.css`**：组件里不写颜色，也不新增 `--ui-*` 平行层（同第 0 节）。
2. **亮色模式是推导的**：参考站只有暗色（亮/暗两次截图逐字节相同，CSS 里没有
   `prefers-color-scheme`），亮色取自同一调色板的亮色成员。因为参考站**只覆盖了暗色**，
   亮色侧的一切**改动后必须人眼过一遍**。
3. **文字档位必须过 AA，且基准固定为「最差背景」**：亮色按最亮的 `#ffffff`、暗色按卡片面
   `#1c1d22` 算（卡片上的字才是正文）。`--td-text-color-placeholder` 在本项目是 `.muted` /
   页脚 / 时间戳的**真实正文**（不是输入框提示），所以取同色阶里仍能读的那一级 ——
   与品牌站最淡那档**刻意不一致**。动这几档前先算对比度，`src/theme.test.ts` 里有断言兜底。
4. **四个语义阶都跟品牌走**（上一版的「告警色保持 TDesign 默认」已被重测推翻）：
   `/result` 用 `#ffc55a` 做 input 的 warning 边框、`#ff5fa0` 做 error 边框，是成体系的
   语义用法。每个语义色都要同时满足「**当文字** ≥4.5:1」与「**当条状图填充** ≥3:1」。
5. **文字色不许取「填充色」token**：`--td-brand-color` 是填充档（按钮底 / 进度条 / 开关），
   它的值是按「上面压白字」选的。拿它当文字色，亮色下侥幸可读（6.57:1）、**按 TDesign
   默认暗色则掉到 2.15:1**，而且所有门禁都是绿的。链接一律走 `--td-text-color-brand` /
   `-link`。本项目实测有三处踩坑（`layout.css` 的 `.models-link` / `.detail-links a`、
   `main.css` 的全局 `a`），**是静态守卫抓出来的、不是人眼**。
   补充：TDesign 把 `--td-brand-color` **同时**当填充与文字用（`variant="base"` 是
   背景、`variant="text"` 与 `.t-menu__item.t-is-active` 是 `color`），同一个色两头都到
   4.5:1 是**算术上无解**的。所以暗色把 `-8` 从 `#0d51d9` 换成 `#4e7ef9`（作文字
   4.55:1 / 白字压它 3.70:1，官方默认同位置 3.76）——**这是唯一一处亮暗不同色的地方**，
   `theme.test.ts` 把两头都钉住，不许换回去。
6. **语义色必须钉在「消费级」上，不是随手挑一级**（条状图发闷的根因）。
   `--td-{success,warning,error}-color` 是**别名**，分别指向 `-5` / `-5` / `-6`（实测
   `es/style/index.css`），而 `.t-progress__inner` 的填充色就是它们。上一版把亮绿
   `#21e786` 钉在 `-8` → 进度条实际拿到 `#1c8b57`，一个发闷的中绿，**而当时所有门禁
   都是绿的**。「条状图用亮色」这件事因此不是选个亮色就完事，是**钉对级别**。
   另一条同源约束：暗色的低端（`-1`..消费级前一档）**不能按「混向页底」造** —— 官方
   暗色阶的低端比页面还亮（是「深色调」不是「页面色」），混页底会做出比官方更深的
   `-1`，而 `-1` 正是 `--td-brand-color-light`（dist 里被消费 27 次，四个 light token
   里最多）。现在暗色低端一律从官方 `-1` 线性插值到锚点。

**重测口径**（换配色时照做）：用 Playwright 取参考站的**计算后样式**（不是目测）→ 由 anchor
按官方色阶的混白/混黑比例生成 10 级阶 → 对实际选用的色跑全量对比度。三个坑：
① 取色要**按选择器块配对查找**（`blockFrom(sel)` 大括号配对），不要 `indexOf("…dark…,")` ——
实测 TDesign 的暗色块选择器是 `:root.dark,\n:root[theme-mode='dark']`，没有尾逗号，
用拼接串去找会返回 -1、静默生成一整套 NaN；
② 阈值不许自己拍，要**拿 TDesign 官方在同一位置的比值当基准**（如亮色 `variant="light"`
文字压在 `-1` 上官方只有 2.8–2.9:1，硬套 4.5 会把合法取值判红）；
③ 暗色低端**不要混向页底**（理由见规矩 6）。

本地草稿脚本在 `.workbuddy/ref/`（`probe-result.cjs` 取参考站全量计算样式 · `result-ramps.mjs`
生成色阶并自检 · `probe-alias.py` 查别名指向哪一级 · `calibrate-thresholds.mjs` 量官方基准 ·
`probe-layers.py` 比深浅 · `probe-bars.cjs` 验进度条实际填充色），**不入库**（`.workbuddy/` 在
`.gitignore` 里），所以上面记的是方法而非可执行入口。完整清单见 `theme.css` 文件头末尾。

**可复现的部分是门禁**：`src/theme.test.ts` 断言

- 正文档位（primary / secondary / placeholder / brand / link）在亮暗两套下压容器面都过 AA；
- 品牌填充上的**反白字**达标，且品牌填充色**当文字**压在容器面上也过 AA（锁死暗色 `#4e7ef9`）；
- **进度条填充**（即别名指向的 `-5` / `-5` / `-6`）压容器面 ≥4.5、压轨道 ≥3 —— 这条是
  「条状图用亮色」的回归守卫，钉错级会红；
- 暗色**卡片必须比页面浅**（刻意的结构偏离，改方向必须同步改文档）；
- 圆角是阶梯（`medium ≥ 3×default`，`round` 不是 `2px`，`circle` 仍是 `50%`）；
- 字面颜色不得外流到 `theme.css` 之外、前景属性不得直接引用填充档 token；
- `theme.css` 必须是 `main.ts` 的最后一个导入。

改配色先跑 `bun run test:unit`；改 `.vue` 还要跑 `build`（`vp check` 不做模板类型检查）。

### 视觉细节的收敛口径：TDesign 默认 + 项目多数派

凡「具体数值」类的视觉细节（进度条几何、字号），**不引入外部参考的任何取值**，
而是以**本项目当前实现的多数派**为准做收敛。改法的口径统一为：

> 先统计现有各处的实际取值 → 取多数派作为标准 → 少数派向它对齐。

边界：这一节管的是**组件里写不写覆盖**。圆角（以及配色）的**尺度**属于 §0.1，
走的是参考站实测的阶梯（20 / 12 / 10 / 8 / 6 / 2），**在 token 层定一次**，
组件侧同样一个字都不许写。
...
| 显式高度 / 圆角覆盖                   | 0    | 不覆盖，几何用 TDesign 默认、圆角用 §0.1 的 token |

即：进度条只统一这三个属性，**不写任何高度/圆角覆盖**。

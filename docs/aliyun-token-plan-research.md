...
该 ticket 的值里含**字面 `$`**（形如 `…EXAMPLEtk$a1b2$FAKE1234567890abc*…`），
而本地环境加载器**都会做 `$VAR` 变量展开**——实测 Bun 的 `.env` 加载器与
Vite 的 `loadEnv()`（`vite.config.ts` 用的就是它）**行为完全一致**：

> 上面的片段是**编造的占位值**，只用来演示两个 `$` 段的位置与长度；
> 下面表格里的段长与真实 ticket 一致，所以「丢 23 个字符」的结论仍然成立。

| 写法                      | 结果                                         |
| ------------------------- | -------------------------------------------- |
| 裸 `$a1b2`                | 被当作变量名，**展开为空**（153 → 130 字符） |
| `\$a1b2`（反斜杠转义）    | ✅ 保留字面 `$`                              |
| `"…$a1b2…"`（双引号包裹） | ❌ 仍然展开（引号不阻止展开）                |
| `'…$a1b2…'`（单引号包裹） | ❌ 仍然展开                                  |
| `${a1b2}`                 | 保留为字面 `${a1b2}`（多了花括号，值仍不对） |

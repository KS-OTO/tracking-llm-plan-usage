...
| 火山方舟 Agent Plan   | `GetPersonalPlan` + `GetAFPUsage` + `GetUsageDetails` + `GetCodingPlanUsage` + `GetInferenceUsage`                                                                                              | 卡片上是 Agent Plan 的 5 小时/每周/每月配额与进度、重置倒计时；模型日额度（只对图片/视频/语音与 Harness 生效）、套餐档位与有效期、Coding Plan 状态与窗口额度、模型调用明细、近 N 天推理用量（可按模型过滤）全部收进「详情」弹窗 |
...
server/           平台无关 API 核心 + Bun 入口
  ...
  volc.ts         方舟管控面 API 客户端（GetPersonalPlan / GetAFPUsage / GetCodingPlanUsage / GetUsageDetails / GetInferenceUsage）
